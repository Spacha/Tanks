...
for i in range(N):
    ...
    points.append([Vector(randint(10,SCR_WIDTH-10), randint(SCR_HEIGHT/2,SCR_HEIGHT-10)), False, False])
...
while running:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            if event.key == pg.K_SPACE:
                free_rect = not free_rect
                if not free_rect:
                    poly_rect.velocity = Vector(0,0)
    ...
    if free_rect:
        if not poly_rect.collides:
            poly_rect.velocity.y += delta * 9.81
        else:
            poly_rect.velocity.y = 0.0

    poly_rect.position += delta * poly_rect.velocity
